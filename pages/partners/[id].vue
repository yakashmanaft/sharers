...
const usersFlipper = (user, actionType, usersArray) => {
    ...
    let prevUserID = usersArray.indexOf(user) ? usersArray[usersArray.indexOf(user) - 1].id : null
    let nextUserID = usersArray.indexOf(user) < usersArray.length - 1 ? usersArray[usersArray.indexOf(user) + 1].id : null
...
const sumCommunityTax = () => {
    return Object.values(usersInBand.value).reduce((acc, current) => {
        if(current.category !== '#2') {
            ...
            acc += ((current.hours * current.stakeIndex) * wageRate.value) - current.salary
        }
        return +acc.toFixed(2)
    }, 0)
}
  ...
  <Container v-if="user">
    ...
    <div class="page-title">
        ...
        <div class="page-title__btn-container">
            <span :class="{'btn__disabled': !isFlipperPrevUserBtnExist}" @click="usersFlipper(user, 'prev', users)">Prev</span>
            <span :class="{'btn__disabled': !isFlipperNextUserBtnExist}" @click="usersFlipper(user, 'next', users)">Next</span>
        </div>
    </div>
    ...
    <div v-if="user.role === 'MASTER'">
        ...
        <p>ФОТ (начисленный) <span>{{ wageFund }}</span></p>
        <p>ФОТ (по выработке) <span>{{ productionFund }} | {{ +(productionFund - wageFund).toFixed(2) }}</span></p>
        ...
        <table style="width: 100%;">
            ...
            <tr v-for="item in usersInBand" :key="item.id" style="">
                ...
                <td>
                    <!-- Час * КТУ -->
                    <span v-if="item.category !== '#2'">{{ (item.hours * item.stakeIndex).toFixed(2) }}</span>
                    <span v-else>-</span>
                </td>
                <td>
                    <!-- ЗП (выработка) -->
                    <span v-if="item.category !== '#2'">{{ ((item.hours * item.stakeIndex) * wageRate).toFixed(2) }}</span>
                    <span v-else>-</span>
                </td>
                <td>
                    <!-- ЗП (начисленная) -->
                ...
                <td>
                    <!-- НАЛОГ -->
                    <span v-if="item.category !== '#2'">{{ (((item.hours * item.stakeIndex) * wageRate) - item.salary).toFixed(2) }}</span>
                    <span v-else>-</span>
                </td>
            </tr>
